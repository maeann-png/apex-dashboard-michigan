...
ENDPOINT = os.getenv("LEAFLINK_ENDPOINT", "/orders-received/")

# Page size. LeafLink allows 1..500; it defaults to 50 if unset.
...
def fetch_all() -> list:
    ...
    # First request: build the URL with our page size. Subsequent requests follow
    # the fully-formed `next` URL LeafLink returns, so we don't re-pass params.
